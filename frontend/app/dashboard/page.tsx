...
import { ArrowLeft, Users, Clock, FileSpreadsheet, UserPlus, Search, Upload, X } from "lucide-react"
...
export default function DashboardPage() {
  ...
  const handleRegistration = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setRegistrationForm({ student_name: "", index_number: "", class: "" })
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600">Manage students, attendance, and model training</p>
          </div>
        </div>
                  ...
                  <CardHeader>
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                      <div>
                        ...
                        <CardDescription>Total Students: {currentClass.students.length}</CardDescription>
                      </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    {currentClass.students.length === 0 ? (
                      <div className="text-center py-8 text-gray-500">No attendance records found for this class.</div>
                    ) : (
          ...
          <TabsContent value="register" className="space-y-6">
            <Card>
              ...
              <CardContent>
                ...
                {registrationResult && (
                  <Alert
                    className={`mt-4 ${registrationResult.message.includes("successfully") ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}`}
                  >
                    <AlertDescription
                      className={
                        registrationResult.message.includes("successfully") ? "text-green-800" : "text-red-800"
                      }
                    >
                      <div className="font-semibold">{registrationResult.message}</div>
                      {registrationResult.name && (
                        <div className="text-sm mt-1">
                          Name: {registrationResult.name} | Index: {registrationResult.index_number}
                        </div>
                      )}
                    </AlertDescription>
                  </Alert>
                ...
                {selectedStudent && (
                  <Alert className="border-blue-200 bg-blue-50">
                    <AlertDescription className="text-blue-800">
                      <div className="font-semibold">Selected: {selectedStudent.student_name}</div>
